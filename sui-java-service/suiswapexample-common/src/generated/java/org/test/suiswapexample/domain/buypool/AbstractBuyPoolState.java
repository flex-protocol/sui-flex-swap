...
import org.test.suiswapexample.domain.buypool.BuyPoolEvent.*;

public abstract class AbstractBuyPoolState implements BuyPoolState.SqlBuyPoolState {
    ...
    public AbstractBuyPoolState(List<Event> events) {
        initializeForReapplying();
        if (events != null && events.size() > 0) {
            this.setId(((BuyPoolEvent.SqlBuyPoolEvent) events.get(0)).getBuyPoolEventId().getId());
            for (Event e : events) {
                mutate(e);
                this.setOffChainVersion((this.getOffChainVersion() == null ? BuyPoolState.VERSION_NULL : this.getOffChainVersion()) + 1);
            }
        }
    }


    public AbstractBuyPoolState() {
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof BuyPoolState) {
            return Objects.equals(this.getId(), ((BuyPoolState)obj).getId());
        }
        return false;
    }
    ...
    public void mutate(Event e) {
        ...
        if (false) { 
            ;
        } else if (e instanceof AbstractBuyPoolEvent.BuyPoolExchangeRateUpdated) {
            when((AbstractBuyPoolEvent.BuyPoolExchangeRateUpdated)e);
        } else if (e instanceof AbstractBuyPoolEvent.BuyPoolDestroyed) {
            when((AbstractBuyPoolEvent.BuyPoolDestroyed)e);
        } else {
            throw new UnsupportedOperationException(String.format("Unsupported event type: %1$s", e.getClass().getName()));
        }
    }

    public void merge(BuyPoolState s) {
        if (s == this) {
            return;
        }
    ...
    public void when(AbstractBuyPoolEvent.BuyPoolExchangeRateUpdated e) {
        throwOnWrongEvent(e);
...
//    public static BuyPoolState mutate(BuyPoolState buyPoolState, String liquidityTokenId, Long suiTimestamp, String suiTxDigest, BigInteger suiEventSeq, String suiPackageId, String suiTransactionModule, String suiSender, String suiType, String status, MutationContext<BuyPoolState, BuyPoolState.MutableBuyPoolState> mutationContext) {
//    }
//}
    ...
    protected void throwOnWrongEvent(BuyPoolEvent event) {
        String stateEntityId = this.getId(); // Aggregate Id
        String eventEntityId = ((BuyPoolEvent.SqlBuyPoolEvent)event).getBuyPoolEventId().getId(); // EntityBase.Aggregate.GetEventIdPropertyIdName();
    ...
    public static class SimpleBuyPoolState extends AbstractBuyPoolState {

        public SimpleBuyPoolState() {
        }

        public SimpleBuyPoolState(List<Event> events) {
            super(events);
        }

        public static SimpleBuyPoolState newForReapplying() {
            SimpleBuyPoolState s = new SimpleBuyPoolState();
