...
module sui_swap_example::buy_pool_initialize_buy_pool_logic {
    use std::option;
    use std::string;
    use std::type_name;
    ...
    use sui_swap_example::buy_pool_initialized;
    ...
    public(friend) fun verify<X: key + store, Y>(
        ...
        buy_pool::new_buy_pool_initialized<X, Y>(
            object::id(exchange),
    ...
    public(friend) fun mutate<X: key + store, Y>(
        buy_pool_initialized: &mut buy_pool::BuyPoolInitialized,
        y_amount: Balance<Y>,
        exchange: &mut Exchange,
        ctx: &mut TxContext,
        ...
        buy_pool::set_buy_pool_initialized_liquidity_token_id(buy_pool_initialized, liquidity_token_id);

        let start_exchange_rate_numerator = exchange_rate_numerator;
        let buy_pool = buy_pool::new_buy_pool<X, Y>(
            x_reserve,
            x_amounts,
            x_total_amount,
            liquidity_token_id,
        ...
        exchange_aggregate::add_buy_pool<X, Y>(exchange, buy_pool::id(&buy_pool), ctx);

        let y_reserve = buy_pool::borrow_mut_y_reserve(&mut buy_pool);
