...
module sui_swap_example::token_pair {
    ...
    public fun assert_schema_version<Y>(token_pair: &TokenPair<Y>) {
        assert!(token_pair.schema_version == SCHEMA_VERSION, EWrongSchemaVersion);
    }

    struct TokenPair<phantom Y> has key {
        id: UID,
        version: u64,
        schema_version: u64,
        admin_cap: ID,
    ...
    public fun id<Y>(token_pair: &TokenPair<Y>): object::ID {
        object::uid_to_inner(&token_pair.id)
    }

    public fun version<Y>(token_pair: &TokenPair<Y>): u64 {
        token_pair.version
    }
    ...
    public(friend) fun borrow_mut_x_reserve<Y>(token_pair: &mut TokenPair<Y>): &mut Movescription {
    ...
    public(friend) fun borrow_mut_y_reserve<Y>(token_pair: &mut TokenPair<Y>): &mut Balance<Y> {
        &mut token_pair.y_reserve
    }

    public fun total_liquidity<Y>(token_pair: &TokenPair<Y>): u64 {
        token_pair.total_liquidity
    }

    public(friend) fun set_total_liquidity<Y>(token_pair: &mut TokenPair<Y>, total_liquidity: u64) {
        token_pair.total_liquidity = total_liquidity;
    }

    public fun admin_cap<Y>(token_pair: &TokenPair<Y>): ID {
    ...
    entry fun migrate<Y>(token_pair: &mut TokenPair<Y>, a: &AdminCap) {
    ...
    public(friend) fun new_liquidity_initialized<Y>(
        exchange_id: ID,
        provider: address,
        x_token_type: String,
        y_token_type: String,
    ...
    #[allow(unused_type_parameter)]
    public(friend) fun new_liquidity_removed<Y>(
        token_pair: &TokenPair<Y>,
        liquidity_amount: u64,
        liquidity_token_id: ID,
        provider: address,
    ...
    #[allow(unused_type_parameter)]
    public(friend) fun new_x_swapped_for_y<Y>(
        token_pair: &TokenPair<Y>,
        expected_y_amount_out: u64,
        sender: address,
        x_token_type: String,
    ...
    #[allow(unused_type_parameter)]
    public(friend) fun new_y_swapped_for_x<Y>(
        token_pair: &TokenPair<Y>,
        expected_x_amount_out: u64,
        sender: address,
        x_token_type: String,
    ...
    public(friend) fun transfer_object<Y>(token_pair: TokenPair<Y>, recipient: address) {
        assert!(token_pair.version == 0, EInappropriateVersion);
        transfer::transfer(token_pair, recipient);
    }

    public(friend) fun update_version_and_transfer_object<Y>(token_pair: TokenPair<Y>, recipient: address) {
        update_object_version(&mut token_pair);
        transfer::transfer(token_pair, recipient);
    }

    #[lint_allow(share_owned)]
    public(friend) fun share_object<Y>(token_pair: TokenPair<Y>) {
        assert!(token_pair.version == 0, EInappropriateVersion);
        transfer::share_object(token_pair);
    }

    #[lint_allow(freeze_wrapped)]
    public(friend) fun freeze_object<Y>(token_pair: TokenPair<Y>) {
        assert!(token_pair.version == 0, EInappropriateVersion);
        transfer::freeze_object(token_pair);
    }

    #[lint_allow(freeze_wrapped)]
    public(friend) fun update_version_and_freeze_object<Y>(token_pair: TokenPair<Y>) {
        update_object_version(&mut token_pair);
        transfer::freeze_object(token_pair);
    }

    public(friend) fun update_object_version<Y>(token_pair: &mut TokenPair<Y>) {
